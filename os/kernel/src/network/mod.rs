...
pub fn open_udp() -> SocketHandle {
    ...
    let rx_buffer = udp::PacketBuffer::new(vec![udp::PacketMetadata::EMPTY, udp::PacketMetadata::EMPTY], vec![0; 65535]);
    let tx_buffer = udp::PacketBuffer::new(vec![udp::PacketMetadata::EMPTY, udp::PacketMetadata::EMPTY], vec![0; 65535]);
...
pub fn close_socket(handle: SocketHandle) {
    let sockets = SOCKETS.get().expect("Socket set not initialized!");
    check_ownership(handle);
    SOCKET_PROCESS.write().remove(&handle);
...
/// Close all sockets for a process by process ID
/// This should be called from process exit/kill, NOT from Drop
pub(crate) fn close_sockets_for_process_id(process_id: usize) {
    // Check if SOCKETS is initialized
    let sockets_lock = match SOCKETS.get() {
        Some(s) => s,
        None => return, // Network not initialized, nothing to clean up
    };

    // Acquire both write locks FIRST to prevent concurrent modifications
    // This is critical to avoid BTreeMap corruption
    let mut socket_process_lock = SOCKET_PROCESS.write();
    let mut sockets = sockets_lock.write();

    // Collect handles while holding write lock (safe because we own the lock)
    let handles_to_remove: Vec<SocketHandle> = socket_process_lock
        .iter()
        .filter_map(|(handle, proc)| if proc.id == process_id { Some(*handle) } else { None })
        .collect();

    // Remove from both maps while holding locks
    for handle in handles_to_remove {
        socket_process_lock.remove(&handle);
        sockets.remove(handle);
    }
}

/// Legacy function for Drop - does nothing now as cleanup happens during exit
pub(crate) fn close_sockets_for_process(_process: &mut Process) {
    // Cleanup now happens in process_manager exit/kill, not in Drop
    // This is intentionally empty to avoid complex operations during Drop
}
